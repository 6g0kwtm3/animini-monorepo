...
- **Assertion** — compare actual to expected. Skip the assertion when the action implies it (e.g. `render(<X />)` implies _X rendered_). See [assertions.md](references/assertions.md) for what to assert, what to skip, and how to handle the negative.
...
**Don't over-mock.** A test that mocks its own internal collaborators is testing the shape of the implementation. The boundary is a chisel; chisel enough and nothing is left to test. If you're mocking something you also wrote, stop — the boundary is in the wrong place. See [boundaries.md](references/boundaries.md) for where to draw the line between unit and integration.
...
- **Flaky in CI** — apply **S.M.A.R.T.** the moment you spot it: Skip → Mitigate → Assess → Rewrite → Throw away. Skip first, investigate second. A flaky test in the suite poisons trust in every other test. See [flaky.md](references/flaky.md).
...
- A coverage target — code coverage is a tool, not a goal. A 100%-covered implementation-detail test is worse than a 50%-covered intention test. See [coverage.md](references/coverage.md).
