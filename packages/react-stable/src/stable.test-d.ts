...
it("law 3: Stable<Stable<T>> is Stable<T>", () => {
	void function test<T>(value: Stable<Stable<T>>): Stable<T> {
		return value
	}
	void function testDeepStable<T>(
		value: DeepStable<DeepStable<T>>
	): DeepStable<T> {
		return value
	}
	assertType(1)
...
it("law 3: Stable<T> is idempotent for every kind of T", () => {
	...
	stableOf<string | undefined | { a: 1 }>("idempotent")
	...
	deepStableOf<string | undefined | { a: 1 }>("idempotent")
	...
	deepStableOfStableOf<string | undefined | { a: 1 }>("idempotent")
})
...
it("Stable<T> of a union keeps its primitive members unmarked", () => {
	assertType<Stable<string | undefined | { a: 1 }>>("foo")
	assertType<Stable<string | undefined | { a: 1 }>>(undefined)
	assertType<Stable<string | undefined | { a: 1 }>>(stable({ a: 1 as const }))
	// @ts-expect-error TS2345 - A freshly created object is not stable.
	assertType<Stable<string | undefined | { a: 1 }>>({ a: 1 })
})
...
it("Stable<T | undefined> narrows to Stable<T>", () => {
	...
	void function testDeepStable<K extends string>(
		...
		assertType<undefined | { wrapped: DeepStable<Record<K, number>> }>(
			value && { wrapped: value }
		)
	}
})
