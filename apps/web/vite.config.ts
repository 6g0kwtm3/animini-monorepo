...
function createReactCompilerPlugin(
  ...
  function isFastRefreshEnabled() {
    return !config.isProduction && config.command !== "build" && config.server.hmr !== false;
  }
  ...
  return {
    ...
    configResolved(resolvedConfig) {
      sourcemap = resolvedConfig.command !== "build" || !!resolvedConfig.build.sourcemap;
    ...
    transform: {
      ...
      async handler(code, id) {
        ...
        if (result.fatal) {
          void this.error(diagnostics.join("\n\n") || "React Compiler transform failed.");
        }
