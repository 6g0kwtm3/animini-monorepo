...
const createMenu = tv({
	slots: {
		...
		list: "max-h-(--popover-avalible-height) open:starting:-translate-y-4 open:starting:opacity-0 allow-discrete right-[anchor(var(--anchor)_right)] top-[anchor(var(--anchor)_bottom)] z-50 flex min-w-[7rem] max-w-[17.5rem] translate-y-12 flex-col overflow-visible overscroll-contain rounded-xs bg-surface-container py-2 text-label-lg text-on-surface opacity-0 duration-4sm ease-emphasized-accelerate [position-try-options:flip-block,flip-inline] open:transform-none open:opacity-100 motion-safe:transition-all",
	},
})
