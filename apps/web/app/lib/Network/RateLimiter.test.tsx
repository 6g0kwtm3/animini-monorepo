...
void describe("Basic functionality", () => {
  it("should execute functions immediately when within rate limit", async () => {
    const limiter = new RateLimiter([{ limit: 3, per: new Temporal.Duration(0, 0, 0, 0, 0, 1) }]);
  ...
  it("should delay execution when rate limit is exceeded", async () => {
    const limiter = new RateLimiter([{ limit: 2, per: new Temporal.Duration(0, 0, 0, 0, 0, 1) }]);
...
void describe("Concurrency", () => {
  it("should handle concurrent execute calls", async () => {
    const limiter = new RateLimiter([{ limit: 1, per: new Temporal.Duration(0, 0, 0, 0, 0, 1) }]);
...
void describe("Edge cases", () => {
  ...
  it("should handle large number of queued items", async () => {
    const limiter = new RateLimiter([{ limit: 5, per: new Temporal.Duration(0, 0, 0, 0, 0, 1) }]);
